<template>
  <div class="articles-section" ref="articleSectionRef">
    ...
    <div class="article-section__list">
      <BlockCasesArticleCard
          :cases-article-list-data="modelsData"
...
import type {CasesCollectionPaginateResponse} from "~/types/responses/collections/cases";
import type {BlogsCollectionPaginateResponse} from "~/types/responses/collections/blogs";

type SectionCasesArticleProps = {
  categories: CategoriesBase[],
  sorting: Sortings[],
  generalButton: string,
}
const props = defineProps<SectionCasesArticleProps>();
...
let modelsData = ref<CasesCollectionPaginateResponse | any>(null)
...
const mountedUrl = computed(() => {
  let url = apiUrlFormat(apiUrl.casesPagination, {page: paginationData.value.page, pageSize: paginationData.value.pageSize});
  ...
  if (filterParam.value !== null) {
    url += `&filters[categories][$eq]=${filterParam.value}`;
  }
  return url;
});
...
const { data, error } = await useFetch<CasesCollectionPaginateResponse>(
    apiUrlFormat(apiUrl.casesPagination, {page: 1, pageSize: 9}),
...
async function getModelsData() {
  const fetchedData = await $fetch<CasesCollectionPaginateResponse>(
...
.article-section__list {
  ...
  lg:grid-cols-2 lg:gap-[13px] lg:mb-[30px]
  sm:grid-cols-1 sm:gap-[10px] sm:mb-[30.5px]
}
</style>
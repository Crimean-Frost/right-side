    ...
    <div class="modal" @click.stop>
      ...
      <div class="modal__inner">
        <div class="modal__content">
          <picture>
            <img :src="`${config.public.apiBase}${data?.data?.modalWindow?.logo?.url}`"
                 :alt="data?.data?.modalWindow?.logo?.alternativeText" class="modal__logo"
            >
          </picture>
          <h3 class="modal__content-title">
            {{ typograf.execute(data?.data?.modalWindow?.title) }}
          </h3>
          <form class="modal__form" @submit.prevent="handleSubmit">
            <div class="modal__form-item">
              <LabelDefault class="modal__form-label" :for="data?.data?.modalWindow?.form.fullName.id">
                {{ data?.data?.modalWindow?.form.fullName.label }}
              </LabelDefault>
              <InputDefault
                  :id="data?.data?.modalWindow?.form.fullName.id"
                  :placeholder="data?.data?.modalWindow?.form?.fullName.placeholder"
                  v-model="form.fullName"
                  :class="['modal__form-input', {'form__input-error': formErrors.isName}]"
              />
            </div>
            <div class="modal__form-item">
              <LabelDefault class="modal__form-label" :for="data?.data?.modalWindow?.form.phone.id">
                {{ data?.data?.modalWindow?.form.phone.label }}
              </LabelDefault>
              <InputDefault
                  :id="data?.data?.modalWindow?.form.phone.id"
                  :placeholder="data?.data?.modalWindow?.form.phone.placeholder"
                  v-model="form.phone"
            ...
            <InputCheckboxDefault
                ...
                :el-id="`wright-me`"
                :text="data?.data?.modalWindow?.form.writeMe || ''"
                v-model="form.writeMe"
            />
            <InputCheckboxDefault
                v-if="agreementText"
                class="modal__form-checkbox"
                :el-id="data?.data?.modalWindow?.form?.agreement?.id.toString()"
                :text="agreementText"
            ...
            <ButtonModalApplication
                ...
                :text="data?.data?.modalWindow.form.button"
            />
          </form>
        </div>
      </div>
...
import type {ModalWindowResponse} from "~/types/responses/modal-window";
...
const isModalOpen = computed(() => store.isModalApplicationOpen)

const {data, error} = await useFetch<ModalWindowResponse>(
    apiUrl.modal,
...
const agreement = data.value?.data.modalWindow.form.agreement;
if (agreement) {
  agreementText.value = agreementFunc(agreement, config.public.apiBase);
}

const closeModal = () => {
  store.toggleIsModalApplicationOpen();
}
